...
<script>
	...
	function editEntry(id, reqname, script, host, status, timer, username, password, port, th, contacts){	
		...
		if(yes){								
			window.location.href="edit_process.php?id="+ id; 					
			//window.location.href="edit_process.php?id="+ id + "&req=" + reqname + "&script=" + script+ "&host=" + host + "&status=" + status + "&timer=" + timer +
			//			"&username=" + username + "&password=" + password + "&port=" + port + "&th=" + th + "&contacts=" + contacts + "&args=" + args;
		}		
	}
...
<table>
  ...
  <tr>
	...
	<table border="1" cellspacing="0" width="950" style="table-layout:fixed">
	  <tbody align="center" style="background-color:#7E587E; font-size:12px; color:#FFFFFF">
		  ...
		  <tr height="30">
			...
			<td width="22%"><b>PROCESS/SCRIPT</b></td>
			<td width="12%"><b>ARGS</b></td>
			<td width="11%"><b>HOST</b></td>
			<td width="7%"><b>STATUS</b></td>
			<td width="9%"><b>ENTRY TIME</b></td>
			<td width="9%"><b>NEXT RUN TIME</b></td>
	  ...
	  <tbody align="center" style="font-size:12px">
		...
		<?php
			...
			$query = "SELECT id, request_token, script, host, status, entry_time, next_run_time, timer, username, password, port, th, contacts, args
			...
			{					
					...
					echo($str."						
						...
						<td style=\"word-wrap: break-word\">$data[13]</td>
